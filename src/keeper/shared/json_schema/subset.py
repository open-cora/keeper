...
`unit` is a custom annotation keyword (see the units convention in
docs/reference/conventions.md): when present on a numeric property its
...
def check_subset(
    ...
    *,
    ...
    for prop_name, prop_schema in properties.items():
        ...
        # declaration; only the root carries it. $schema is allowed
...
def check_schema_is_subset(
    ...
    *,
    ...
    The cross-schema subset relation for a two-tier declaration, where a
    narrower schema must stay inside a broader one it specializes. Both
    sides use the same constrained subset (`ALLOWED_SCHEMA_KEYS`); the
    ...
    posture per the schema-validated-values pattern in docs/reference/conventions.md):
      ...
      3. inner.required must be a subset of outer's declared
         property keys: the inner schema cannot require a property
         the outer never declared.
    ...
    Tolerance: a key present in outer but absent in inner is always
    allowed. The inner schema need not mention every property the outer
    declares; only `required` on the inner side propagates.
    ...
    if extra_props:
        msg = (
            ...
            f"that outer does not declare; the inner schema may not introduce "
            f"properties outside the outer contract"
        )
        raise error_class(msg)
    ...
    # 8. unit exact match (per the units convention in docs/reference/conventions.md)
