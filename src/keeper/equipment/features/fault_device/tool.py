"""MCP door for recording a device fault.
...
class FaultDeviceOutput(BaseModel):
...
def register(mcp: FastMCP, *, get_handler: Callable[[], Handler]) -> None:
    ...
    @mcp.tool(
        name="fault_device",
        description=(
            "Record that a device faulted. Deciding that a given alarm amounts to a "
            "fault is the caller's judgement. Refused if the device is already "
            "faulted or has been retired."
        ),
    )
    async def fault_device_tool(  # pyright: ignore[reportUnusedFunction]
        ...
        handler = get_handler()
        await handler(
            FaultDevice(device_id=device_id, occurred_at=occurred_at),
        ...
        return FaultDeviceOutput(device_id=device_id)
