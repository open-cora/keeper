...
class ListenNotifyWakeup:
    ...
    async def _ensure_listening(self) -> None:
        ...
        The failure has two halves and they are not equally visible.
        The `InterfaceError`s are transient and self-healing, so they
        look like the whole story. The leaked connections are the part
        that persists: each loser of the race parks on `LISTEN` for the
        lifetime of the process, so the pool shrinks by one per racing
        projection until the next restart.
