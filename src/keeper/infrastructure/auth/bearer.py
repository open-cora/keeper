...
request it:
  ...
  2. Skips when no `TokenVerifier` is configured on the kernel
     ...
     stays in effect; see `keeper.infrastructure.request.get_principal_id`).
...
from keeper.infrastructure.observability.surface_context import (
...
if TYPE_CHECKING:
    from keeper.infrastructure.ports import TokenVerifier
...
# `keeper.infrastructure.auth` which is mid-load. Pinning the
        ...
        *,
        ...
        if authorization is None:
            ...
            if routed_path == "/mcp" or routed_path.startswith("/mcp/"):
                ...
                from keeper.infrastructure.auth.exception_handlers import (
        ...
        # keeper.infrastructure and would trigger the auth-package-init
...
def _extract_bearer_token(authorization_header: str) -> str:
    ...
    from keeper.infrastructure.ports import InvalidTokenError
